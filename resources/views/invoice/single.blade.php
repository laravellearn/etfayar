                        ...
                        <div class="row justify-content-center bgi-size-cover bgi-no-repeat py-8 px-8 py-md-27 px-md-0"
                            ...
                            <div class="col-md-9">
                                <div class="d-flex justify-content-center pb-10 pb-md-20 flex-column">
                                    <h1 class="display-4 text-white font-weight-boldest mb-5 text-center">
                                        فاکتور</h1>
                                    <h4 class="text-white mb-3 text-center">{{$single->title}}</h4>
                          ...
                          {{--  <span>شرکت فناوری اطلاعات ساتراس</span>
                            <span>راست چین وب</span>--}}
                        </span>
                                    </div>
                                </div>
                                <div class="border-bottom w-100 opacity-20"></div>
                                <div class="d-flex justify-content-between text-white text-center pt-6">
                                    <div class="d-flex flex-column flex-root  text-black">
                                        <span class="font-weight-bolder mb-2 text-black">تاریخ</span>
                                        <span class="opacity-70">{{$single->persianDateTime}}</span>
                                    </div>
                                    <div class="d-flex flex-column flex-root">
                                        <span class="font-weight-bolder mb-2">شماره فاکتور</span>
                                        <span class="opacity-70">{{$single->code}}</span>
                                    </div>
                                    <div class="d-flex flex-column flex-root">
                                        <span class="font-weight-bolder mb-2">آدرس خریدار</span>
                                        @if(isset($single->request->user->address))
                                            <span
                                                    class="opacity-70">{{$single->request->user->address->city->province->name}} {{$single->request->user->address->city->name}}</span>
                                            <span class="opacity-70">{{$single->request->user->address->area}}</span>
                                            <span class="opacity-70">{{$single->request->user->address->address}}</span>
                                            <span
                                                    class="opacity-70"> کد پستی : {{$single->request->user->address->postal_code}}</span>
                                        @else
                                            <span class="opacity-70">ثبت نشده است</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="d-flex justify-content-between flex-column flex-md-row font-size-lg">
                                    <div class="d-flex flex-column mb-10 mb-md-0">
                                        ...
                                        <div class="d-flex justify-content-between mb-3">
                                            ...
                                            <span class="text-right">{{$single->request->user->customer_code??''}}</span>
                                        </div>
