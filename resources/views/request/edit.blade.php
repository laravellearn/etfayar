...
@section('content')
    <div class="col-lg-12">
        ...
        <div class="card card-custom gutter-b example example-compact">
            ...
            <!--begin::Form-->
            <form class="form" action="{{route('request.update')}}" method="post">
                <div class="card-body">
                    @csrf
                        ...
                        <div class="col-9">
                            <select class="form-control" name="service_id">
                                ...
                                @foreach($services as $item)
                                    <option
                                        value="{{$item->id}}" {{$request->service_id == $item->id ? 'selected' : '' }} >{{$item->title}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-9">
                            <select class="form-control selectpicker" name="user_id" data-size="5"
                                ...
                                @foreach($users as $item)
                                    <option
                                        value="{{$item->id}}" {{$request->user_id == $item->id ? 'selected' : '' }} >#{{ $item->customer_code }}     {{ $item->full_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-9">
                                                <div class="input-group">
                                                    ...
                                                    <input class="form-control" type="text" name="code" id="code" value="{{$request->code}}" placeholder="@lang("request.code")">
                                                </div>
                                                <span class="form-text text-muted">با کلیک بر روی این دکمه می توانید برای درخواست مشتری خود کد تصادفی ایجاد کنید.</span>
                                            </div>
                                        </div>
                        ...
                        <div class="col-9">
                            ...
                            <select class="form-control" name="status">
                                ...
                                @foreach($items as $item)
                                    <option
                                        {{$request->status ===$item['value'] ? 'selected' : '' }} value="{{$item['value']}}">{{$item['title']}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
